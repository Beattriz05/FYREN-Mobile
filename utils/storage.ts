...
// --- HELPERS ---
const createTimelineEvent = (title: string, desc?: string, icon: string = 'circle'): TimelineEvent => ({
...
export async function saveIncident(incident: Incident): Promise<void> {
  ...
  const initialEvent = createTimelineEvent('Ocorrência Criada', 'Registro inicial no dispositivo.', 'plus-circle');
...
export async function updateIncident(id: string, updates: Partial<Incident>): Promise<void> {
  ...
  if (index !== -1) {
    ...
    if (updates.status && updates.status !== current.status) {
      newTimeline.unshift(createTimelineEvent(
        'Status Atualizado', 
...
export async function getComments(incidentId?: string): Promise<Comment[]> {
  try {
    ...
    return incidentId ? allComments.filter(c => c.incidentId === incidentId) : allComments;
  } catch (error) { return []; }
}
...
export async function getUsers(): Promise<AppUser[]> {
  try {
    ...
    const defaultUsers: AppUser[] = [
      { id: '1', name: 'João Silva', email: 'user@email.com', role: 'user', createdAt: new Date().toISOString(), active: true },
      { id: '2', name: 'Maria Santos', email: 'chief@email.com', role: 'chief', createdAt: new Date().toISOString(), active: true },
      { id: '3', name: 'Admin Fyren', email: 'admin@email.com', role: 'admin', createdAt: new Date().toISOString(), active: true },
    ];
  ...
  setTimeout(async () => {
    try {
      ...
      if (index !== -1 && incidents[index].syncStatus === 'pending_sync') {
        ...
        const syncEvent = createTimelineEvent('Sincronizado', 'Dados enviados ao servidor central.', 'cloud');
